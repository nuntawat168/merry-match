...
function Notification() {
    const notifications = [
        {username: "Khal Drogo", content: "Just Merry you! Click here to see profile"},
        {username: "Daeny", content: "Merry you back! Let's start conversation now"},
        {username: "Ygritte", content: "Merry you back! Let's start conversation now"},
    ]
    ...
    const renderedNoti = notifications.map((noti, index) => {
        return (
            <div key={index} className="flex justify-between py-[12px] px-[14px]">
                ...
                <p className="w-[179px]">'{noti.username}' {noti.content}</p>
            </div>
        )
    })
